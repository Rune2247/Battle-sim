...
import { Teams } from "../battle/js/state.js"
...
class QuadTree {
  ...
  getAllPoints() {
    return this.closest(this.boundary, this.length)
  }
  ...
  kill(point) {
    ...
    if (this.divided) {
      this.children().forEach((tree) => {
        this.killInChildren(point, tree)
      })
    }
  }

  killInChildren(point, tree) {
    tree.points.forEach((p, index) => {
      if (p.id === point.id) {
    ...
    if (tree.divided) {
      tree.children().forEach((tree) => {
        tree.killInChildren(point, tree)
      })
    }
  }
  ...
  kNearest(
    ...
    this.points
      .sort(
        (a, b) => a.sqDistanceFrom(searchPoint) - b.sqDistanceFrom(searchPoint)
      )
      .forEach((p) => {
  ...
  closestEnemy(searchPoint, teamColor, maxCount = 1, maxDistance = Infinity) {
    const enemyTeam = teamColor === Teams.RED ? Teams.BLUE : Teams.RED
    ...
    this.children()
      ...
      .forEach((child) => {
        ...
        if (sqDist > sqMaxDistance) {
          ...
          const result = child.kNearestEnemy(
            searchPoint,
            maxCount,
            sqMaxDistance,
            furthestSqDistance,
...
export default { Point, Rectangle, QuadTree, Circle }
